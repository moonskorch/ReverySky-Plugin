...
/// <summary> 
/// The visual result is very appealing and resembles a starry cosmos. Motion lasts only a few seconds.
/// Local connections are visible here and there, which is nice.
/// It may be worth not stopping the movement at all and letting the links and repulsion converge the layout to a logical equilibrium.
/// Right now, the frozen state is semi-random: some pieces are visibly forced apart and would likely settle together if the forces kept running longer.
/// The hub is very dense. Normal and cluster maps at 2K look acceptable - pleasantly sparse, with some structure, but without excessive density.
/// [Cartographer] Graph built in 718,7 ms (notes=2000, engine=StaticLinks)
/// [Cartographer] Graph built in 1905,4 ms (notes=5000, engine=StaticLinks)
/// [Cartographer] Graph built in 3452,6 ms (notes=10000, engine=StaticLinks)
/// Performance drops to about 10 FPS even on sparse maps. FPS is 7 with only a few nodes visible per frame on a 5K map.
/// Motion reduces FPS by a few more frames.
/// At 10K, FPS is 2.
/// It works with and without tags.
/// Overall: very promising, and the rendering style fits the space theme well. Performance is the problem.
/// It needs further refinement. At minimum, it could become an alternative engine with its own character, not just a force-layout map, but a map of space.
/// </summary>


// Evaluation:
// - Experimental follow-up to Engine_RecursiveHubs_v1.
// - Purpose: keep the hub-first deterministic backbone, but make the layout
//   visibly iterative and prevent cross-link contraction from gluing clusters.
// - Main differences from v1:
//   - no hard layout sphere clamp, so dense maps cannot be flattened against a border;
//   - root clusters reserve explicit guard distance from other root clusters;
//   - refinement alternates link contraction with local/cluster separation;
//   - optional per-frame refinement lets intermediate iterations be observed;
//   - long tag lines are filtered from the visual budget by default.
//
// Assessment:
// - Pending manual evaluation on Normal/Hub/Clusters/Tagless sample matrices.

/// <summary>
/// Iterative hub-cluster layout for medium and large note graphs.
///
/// BuildGraph() creates a deliberately spacious root/backbone placement and
/// instantiates visuals immediately. Tick() then runs a bounded number of
/// refinement passes, pulling real links shorter while preserving minimum
/// distances between nodes and between artificial root clusters.
/// </summary>
[DisallowMultipleComponent]
public class Engine_RecursiveHubs_v2 : MonoBehaviour, ICartographerEngine
{
  ...
  [Header("Initial separated layout")]
  [Tooltip("Primary density control. Larger values create a more spacious initial map.")]
  [SerializeField, Min(0.1f)] private float nodeSpacingFactor = 7.4f;
  [SerializeField, Min(0.1f)] private float minimumNavigationRadius = 14f;
  [SerializeField, Min(0f)] private float navigationPadding = 6f;
  [SerializeField, Range(0.2f, 2f)] private float componentRadiusFactor = 1.08f;
  [SerializeField, Range(0.5f, 4f)] private float componentGapFactor = 1.45f;
  [SerializeField, Range(0.05f, 0.98f)] private float rootSpreadRatio = 0.78f;

  [Header("Structural score")]
  [SerializeField, Min(0f)] private float directLinkScoreWeight = 2.4f;
  [SerializeField, Min(0f)] private float noteTagScoreWeight = 0.55f;
  [SerializeField, Min(0f)] private float degreeScoreBonus = 0.5f;
  [SerializeField, Min(0f)] private float tagNodeScoreMultiplier = 1.35f;
  [Tooltip("Lower than v1 so generic tags do not become gravity wells.")]
  [SerializeField, Min(0.1f)] private float maximumTagHubScore = 6.5f;

  [Header("Diversified roots")]
  [SerializeField, Range(1, 24)] private int maxRootHubsPerComponent = 10;
  [SerializeField, Range(0.1f, 2.5f)] private float rootCountFactor = 0.9f;
  [SerializeField, Range(0, 5)] private int rootExclusionGraphDepth = 2;
  [Tooltip("Avoid choosing tag nodes as roots while note candidates exist.")]
  [SerializeField] private bool preferNotesAsRoots = true;

  [Header("Backbone placement")]
  [SerializeField, Min(0.1f)] private float minimumNodeDistance = 2.7f;
  [SerializeField, Min(0.1f)] private float hubChildDistanceFactor = 2.55f;
  [SerializeField, Min(0.1f)] private float leafChildDistanceFactor = 1.45f;
  [SerializeField, Min(0f)] private float childReservationFactor = 0.92f;
  [SerializeField, Min(0.1f)] private float maximumChildReservation = 16f;
  [SerializeField, Min(0f)] private float siblingShellFactor = 0.58f;
  [SerializeField, Range(2, 64)] private int childHubDegreeThreshold = 4;
  [SerializeField, Range(8, 512)] private int maxPlacementAttempts = 128;

  [Header("Parent selection")]
  [SerializeField, Min(0f)] private float directLinkParentWeight = 3.6f;
  [SerializeField, Min(0f)] private float noteTagParentWeight = 0.75f;
  [SerializeField, Min(0f)] private float parentScoreInfluence = 0.08f;
  [SerializeField, Min(0f)] private float parentCrowdingPenalty = 0.22f;
  [SerializeField, Min(0f)] private float frontierTargetScoreWeight = 4.5f;
  [SerializeField, Min(0f)] private float frontierRelationWeight = 1.15f;

  [Header("Visible iterative refinement")]
  [Tooltip("When true, BuildGraph instantiates the initial layout and Tick animates refinement passes.")]
  [SerializeField] private bool animateRefinement = true;
  [SerializeField, Range(0, 96)] private int refinementPasses = 18;
  [SerializeField, Range(1, 12)] private int refinementPassesPerFrame = 1;
  [SerializeField, Range(0f, 1f)] private float linkPull = 0.095f;
  [SerializeField, Min(0.01f)] private float maxMovePerPass = 0.62f;
  [SerializeField, Min(0.01f)] private float noteTagRestLength = 8.2f;
  [SerializeField, Min(0.01f)] private float directLinkRestLength = 7.0f;
  [SerializeField, Min(0.01f)] private float minimumDirectLinkRestLength = 3.4f;
  [SerializeField, Range(0f, 1f)] private float rootMobility = 0.12f;

  [Header("Anti-glue guards")]
  [Tooltip("Hard local guard applied after every link-contraction pass.")]
  [SerializeField, Range(0, 8)] private int separationPassesPerRefinement = 2;
  [SerializeField, Min(1f)] private float clusterGuardDistanceFactor = 3.2f;
  [SerializeField, Range(8, 8192)] private int maxSeparationChecksPerNode = 160;

  [Header("Visual")]
  [SerializeField, Min(0.01f)] private float tagScale = 0.7f;
  [SerializeField, Min(0)] private int maxVisibleEdges = 1500;
  [SerializeField, Range(0f, 1f)] private float backboneBudgetRatio = 0.46f;
  [SerializeField, Range(0f, 1f)] private float directLinkBudgetRatio = 0.42f;
  [Tooltip("Filters weak-looking tag spokes that span across artificial clusters.")]
  ...
  private readonly struct PlacementProposal
  {
    ...
    public PlacementProposal(int to, float priority, long sequence)
    {
      To = to;
      Priority = priority;
      Sequence = sequence;
  ...
  public bool RequiresTick => _remainingRefinementPasses > 0;
  ...
  public void Tick(float dt)
  {
    ...
    if (_remainingRefinementPasses == 0)
    {
      UnityEngine.Debug.Log(
        $"[RecursiveHubs/v2] Animated refinement completed passes={_completedRefinementPasses}, " +
  ...
  public void BuildGraph(List<NoteData> notes)
  {
    ...
    if (_noteCount == 0)
    {
      ...
      totalStopwatch.Stop();
      UnityEngine.Debug.Log(
        $"[RecursiveHubs/v2] Built empty graph in {totalStopwatch.Elapsed.TotalMilliseconds:F1} ms. " +
    ...
    int safeRefinementPasses = Mathf.Clamp(refinementPasses, 0, 96);
    ...
    _remainingRefinementPasses = animateRefinement ? safeRefinementPasses : 0;

    totalStopwatch.Stop();

    UnityEngine.Debug.Log(
      $"[RecursiveHubs/v2] Built notes={_noteCount}, tags={_nodes.Count - _noteCount}, " +
      ...
      $"animated={animateRefinement}, completedRefinementPasses={_completedRefinementPasses}, " +
      $"remainingRefinementPasses={_remainingRefinementPasses}, separationChecks={_separationPairChecks}, " +
      $"visibleEdges={_lineBindings.Count}, navigationRadius={_navigationRadius:F1}, " +
      $"totalMs={totalStopwatch.Elapsed.TotalMilliseconds:F1}, LogicalMs={logicalStopwatch.Elapsed.TotalMilliseconds:F1}, " +
      $"PlaceStructuralHierarchyMs={placementStopwatch.Elapsed.TotalMilliseconds:F1}, " +
      $"RefinementMs={refinementStopwatch.Elapsed.TotalMilliseconds:F1}, " +
  ...
  {
    _placementCells.Clear();
    _packingOffsets = BuildPackingOffsets(Mathf.Clamp(maxPlacementAttempts, 8, 512));
  ...
  private void PlaceComponent(int componentIndex)
  {
    ...
    while (placedCount < component.Nodes.Count)
    {
      ...
      Vector3 fallbackPreferred =
        component.Center +
        StableDirection(_nodes[fallbackRoot].Key, 307) *
        component.Radius *
        Mathf.Clamp(rootSpreadRatio, 0.05f, 0.98f);
  ...
  private List<int> SelectDiversifiedRoots(Component component)
  {
    ...
    if (preferNotesAsRoots && component.Nodes.Any(nodeIndex => _nodes[nodeIndex].IsNote))
  ...
  private void EnqueueProposalsFrom(int nodeIndex)
  {
    ...
    for (int neighborOffset = 0; neighborOffset < neighbors.Count; neighborOffset++)
    {
      ...
      if (target.IsPlaced || target.ComponentIndex != from.ComponentIndex)
        continue;
      ...
      _frontier.Push(new PlacementProposal(
        targetIndex,
        priority,
    ...
    Edge relation)
  {
    ...
    float relationCompression = relation.Kind == EdgeKind.DirectNoteLink
      ? Mathf.Clamp(Mathf.Sqrt(Mathf.Max(0.01f, relation.Weight)), 0.9f, 1.45f)
      : 1f;
    ...
    Vector3 direction =
      ...
      StableDirection(child.Key, 409) * 0.22f;
  ...
  private void ApplyLinkContractionCorrections()
  {
    ...
    for (int edgeIndex = 0; edgeIndex < _allEdges.Count; edgeIndex++)
    {
      ...
      float weightScale = edge.Kind == EdgeKind.DirectNoteLink
        ? Mathf.Clamp(Mathf.Sqrt(Mathf.Max(0.01f, edge.Weight)), 0.55f, 2.4f)
        : 0.55f;
        ...
        for (int y = -1; y <= 1 && checks < safeMaxChecks; y++)
          for (int z = -1; z <= 1 && checks < safeMaxChecks; z++)
          {
                 ...
                 bucketOffset++)
            {
              ...
              ApplyPairSeparation(nodeIndex, otherIndex, localMinimum, clusterGuard);
            }
          }
    }
  ...
  private void InstantiateNodes()
  {
    ...
    {
      UnityEngine.Debug.LogError("[RecursiveHubs/v2] Missing starTemplate.");
      return;
    }

    bool canCreateTags = tagNodeTemplate != null;
    if (!canCreateTags && _nodes.Count > _noteCount)
      UnityEngine.Debug.LogWarning("[RecursiveHubs/v2] Missing tagNodeTemplate. Tag nodes were skipped.");
