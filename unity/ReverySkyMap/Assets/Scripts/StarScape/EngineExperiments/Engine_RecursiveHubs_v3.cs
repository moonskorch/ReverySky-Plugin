...
/// <summary>
/// Even more natural and more like a star map than v2. It looks very good. There is almost no sticking together.
/// The animation appears in steps at first, then runs briefly, and then freezes, which is interesting.
/// Local connections are visible.
/// [Cartographer] Graph built in 323,7 ms (notes=2000, engine=StaticLinks)
/// At 2K from a distance, FPS is 15-20. While it is moving, FPS is a few frames lower.
/// It works with and without tags.
/// There are a few awkwardly long one-way links, for example in a hub with two centers: the centers remain separated, and long links stretch between them.
/// On small maps, it can be too sparse (although it resembles a star field), but it looks unattractive because all lines are rendered. The line LOD needs to be reworked.
/// Overall: with refinement, I would prefer this layout type over Barnes, at least up to 2K. It seems even cleaner than v2 of the prototype.
/// FPS still needs to improve. The animation duration and link convergence time could also be increased. The line LOD needs further work.
/// </summary>


// Evaluation:
// - Experimental structural layout, rebuilt from the v1/v2 idea rather than
//   tuned only by constants.
// - Purpose: draw the map around graph structure first, then apply bounded
//   link contraction without allowing links to become glue.
// - Conceptual family: hub-based / multiscale graph drawing. The coarse graph
//   is made from structural maxima, then each placed hub expands into its
//   strongest unplaced neighbors, recursively.
// - Main differences from v2:
//   - placement can be progressive: roots appear first, then frontier waves;
//   - frontier priority is structural, so local top-X hubs lead each wave;
//   - exhausted frontiers spawn a new local maximum instead of dragging a map
//     across empty space;
//   - links are drawn after structural placement, then refinement starts;
//   - no spherical clamp is used for layout positions.
//
// Assessment:
// - Pending manual evaluation on Normal/Hub/Clusters/Tagless sample matrices.

/// <summary>
/// Progressive structural-hub layout for medium and large note graphs.
///
...
public class Engine_RecursiveHubs_v3 : MonoBehaviour, ICartographerEngine
{
  ...
  public void Tick(float dt)
  {
    ...
    if (_remainingRefinementPasses == 0)
    {
      UnityEngine.Debug.Log(
        $"[RecursiveHubs/v3] Refinement completed passes={_completedRefinementPasses}, " +
  ...
  public void BuildGraph(List<NoteData> notes)
  {
    ...
    if (_noteCount == 0)
    {
      ...
      totalStopwatch.Stop();
      UnityEngine.Debug.Log(
        $"[RecursiveHubs/v3] Built empty graph in {totalStopwatch.Elapsed.TotalMilliseconds:F1} ms. " +
    ...
    UnityEngine.Debug.Log(
      $"[RecursiveHubs/v3] Started notes={_noteCount}, tags={_nodes.Count - _noteCount}, " +
  ...
  private void CompleteConstruction()
  {
    ...
    UnityEngine.Debug.Log(
      $"[RecursiveHubs/v3] Construction completed placed={_placedCount}/{_nodes.Count}, " +
  ...
  private void InstantiateNode(int nodeIndex)
  {
    ...
    if (node.IsNote)
    {
      ...
      {
        UnityEngine.Debug.LogError("[RecursiveHubs/v3] Missing starTemplate.");
        return;
      }
