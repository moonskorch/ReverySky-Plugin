...
// Evaluation:
// - Visual defect: the layout collapses into a flattened plate, and the camera ends up inverted relative to the map.
// - Readability: large maps become heavily congested and unreadable; medium maps are only marginally readable.
// [Cartographer] Graph built in 638,1 ms (notes=2000, engine=StaticLinks)
// [Cartographer] Graph built in 4275,0 ms (notes=10000, engine=StaticLinks)
// Assessment:
// - Visually, this variant is similar to Engine_Barnes_v2_FixedCenterIntoPlate, but appears somewhat worse due to higher congestion.
// - To remain worth further evaluation, it needs the plate-shape defect fixed and additional spacing improvements for medium-to-large graphs.
...
public class Engine_Grid_v2_FixedCenterIntoPlate : MonoBehaviour, ICartographerEngine
{
  ...
  [SerializeField, Range(1, 1000)] private int maxIterations = 36;
  ...
  [SerializeField, Min(0.0001f)] private float settleEpsilon = 0.05f;
  ...
  [SerializeField, Min(0.1f)] private float repulsionRadius = 6.5f;
  ...
  [SerializeField, Range(8, 4096)] private int maxRepulsionChecksPerNode = 128;
  ...
  private double _buildSpatialGridMs;
  private double _applyLocalRepulsionMs;
  ...
  public void BuildGraph(List<NoteData> notes)
  {
    ...
    if (_noteCount == 0)
    {
      ...
      UnityEngine.Debug.Log(
        $"[CartographerSettledForces/Grid] BuildGraph totalMs={totalStopwatch.Elapsed.TotalMilliseconds:F1}, " +
    ...
    UnityEngine.Debug.Log(
      $"[CartographerSettledForces/Grid] Built notes={_noteCount}, tags={_nodes.Count - _noteCount}, " +
      ...
      $"navigationRadius={_navigationRadius:F1}, pivotLocal={_layoutCenter}, " +
      ...
      $"BuildSpatialGridMs={_buildSpatialGridMs:F1}, " +
      $"ApplyLocalRepulsionMs={_applyLocalRepulsionMs:F1}, " +
  ...
  public void ClearGraph()
  {
    ...
    _buildSpatialGridMs = 0d;
    _applyLocalRepulsionMs = 0d;
  ...
  private void RelaxLayout()
  {
    ...
    for (int iteration = 0; iteration < safeMaxIterations; iteration++)
    {
      ...
      phaseStopwatch.Stop();
      _buildSpatialGridMs += phaseStopwatch.Elapsed.TotalMilliseconds;

      phaseStopwatch.Restart();
      ApplyLocalRepulsion();
      phaseStopwatch.Stop();
      _applyLocalRepulsionMs += phaseStopwatch.Elapsed.TotalMilliseconds;
