...
- `tests/graph/GraphQueryFilter.test.ts`
  - validates path, date, tag, unsupported-token filter behavior, and always-include center note behavior for Ego scope.
...
- `tests/view/MapSession.test.ts`
  - validates session state normalization, graph refresh timing, filtering, Ego scope, focus precedence, rename focus, and render-scale reopen state.
