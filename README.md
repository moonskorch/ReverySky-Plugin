...
The bundled `.wasm` module is generated by Unity as part of the WebGL build. It contains the compiled Unity runtime and ReverySky map code required to render and run the 3D map inside Obsidian.

The module is built from the Unity project source in `unity/ReverySkyMap/`, bundled with the plugin release, and loaded locally. It is not downloaded from an external server.
