...
describe("MapSession", () => {
  ...
  it("keeps landmark-only changes in the next graph payload", () => {
    ...
    initialPayload.notes[0] = {
      ...initialPayload.notes[0]!,
      buildings: ["Observatory"]
    };
    const buildGraph = makeBuildGraphMock(initialPayload);
    const sendGraph = vi.fn();
    const sendNoteUpdate = vi.fn();
    const session = new MapSession({
      app: app as never,
    ...
    expect(buildGraph).toHaveBeenCalledTimes(1);
    expect(sendGraph).toHaveBeenCalledTimes(2);
    expect((sendGraph.mock.calls[1]?.[0] as GraphPayload).notes[0]?.buildings).toEqual(["Archive"]);
  });
