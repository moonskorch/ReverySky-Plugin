...
// Evaluation:
// - Visual defect: the map takes the form of a plate instead of a sphere, and the camera ends up inverted relative to that layout.
// - Spacing is acceptable at ~2K notes, but the map becomes too congested beyond that range.
// - Cluster separation is distinct, and clusters remain individually readable at smaller scales.
// [Cartographer] Graph built in 1049,0 ms (notes=2000, engine=StaticLinks)
// [Cartographer] Graph built in 5887,6 ms (notes=10000, engine=StaticLinks)
// Assessment:
// - Performance is tolerable, and the visual result is acceptable at smaller graph sizes.
// - The plate-shape defect must be fixed first.
// - This variant did not improve on Engine_Barnes_v1 and lost much of its visual strength, but it may still have value as a large-graph fallback.
// - It remains uncertain whether further refinement is justified even as an intermediate candidate.


/// <summary>
/// Static force-directed map for medium and large note graphs.
///
/// The engine calculates a static force layout once during BuildGraph(), then
...
/// - layout positions are never clamped or scaled to a sphere;
/// - the reported radius is calculated after layout only for camera navigation;
/// - layout iterations, octree traversal, overlap cleanup, and rendered edges are bounded.
/// </summary>
[DisallowMultipleComponent]
public class Engine_Barnes_v2_FixedCenterIntoPlate : MonoBehaviour, ICartographerEngine
{
  ...
  [Header("Initial layout spread")]
  [Tooltip("Primary density control for the deterministic starting layout. Larger values create a more relaxed map.")]
  [SerializeField, Min(0.1f)] private float initialSpacingFactor = 8.5f;
  [SerializeField, Min(0.1f)] private float minimumInitialSpread = 14f;
  ...
  [Tooltip("Used only by camera navigation after layout. It does not constrain physics.")]
  [SerializeField, Min(0.1f)] private float minimumNavigationRadius = 14f;
  [SerializeField, Min(0f)] private float navigationPadding = 4f;

  [Header("Settled relaxation")]
  [SerializeField, Range(1, 1000)] private int maxIterations = 26;
  [SerializeField, Range(0, 1000)] private int minIterations = 10;
  [SerializeField, Range(1, 64)] private int stableIterationsRequired = 2;
  ...
  [SerializeField, Range(1, 1000)] private int largeGraphIterationFloor = 16;
  [SerializeField, Min(0.0001f)] private float settleEpsilon = 0.08f;
  ...
  [Tooltip("Lower is more accurate and slower. Values around 0.9-1.3 are a practical range for a static decorative map.")]
  [SerializeField, Range(0.2f, 1.5f)] private float barnesHutTheta = 1.25f;
  [SerializeField, Range(1, 32)] private int octreeLeafCapacity = 4;
  ...
  [SerializeField, Range(32, 8192)] private int maxBarnesHutVisitsPerNode = 192;
  ...
  private float _initialLayoutRadius;
  ...
  private void Awake()
  {
    _initialLayoutRadius = CalculateInitialLayoutRadius(0, initialSpacingFactor, minimumInitialSpread);
  ...
  public void BuildGraph(List<NoteData> notes)
  {
    ...
    var spreadStopwatch = Stopwatch.StartNew();
    _initialLayoutRadius = CalculateInitialLayoutRadius(
      _nodes.Count,
      initialSpacingFactor,
      minimumInitialSpread);
    spreadStopwatch.Stop();
    ...
    if (_noteCount == 0)
    {
      ...
      UnityEngine.Debug.Log(
        ...
        $"CalculateInitialLayoutRadiusMs={spreadStopwatch.Elapsed.TotalMilliseconds:F1}, notes=0");
      return;
    }
    ...
    UnityEngine.Debug.Log(
      ...
      $"components={_components.Count}, iterations={_settledIterations}, navigationRadius={_navigationRadius:F1}, " +
      $"pivotLocal={_layoutCenter}, octreePeakNodes={_peakOctreeNodes}, treeVisits={_barnesHutNodeVisits}, " +
      $"treeApproximations={_barnesHutApproximations}, exactLeafChecks={_barnesHutExactChecks}, " +
      $"skippedExactLeafBodies={_barnesHutSkippedExactLeafBodies}, cappedTraversals={_barnesHutCappedTraversals}, " +
      $"overlapChecks={_overlapPairChecks}, totalMs={totalStopwatch.Elapsed.TotalMilliseconds:F1}, " +
      $"ClearGraphMs={clearStopwatch.Elapsed.TotalMilliseconds:F1}, " +
      $"BuildLogicalGraphMs={logicalStopwatch.Elapsed.TotalMilliseconds:F1}, " +
      $"CalculateInitialLayoutRadiusMs={spreadStopwatch.Elapsed.TotalMilliseconds:F1}, " +
      ...
      $"UpdateNavigationBoundsMs={navigationStopwatch.Elapsed.TotalMilliseconds:F1}, " +
  ...
  public static float CalculateInitialLayoutRadius(
    int totalNodeCount,
    float spacingFactor,
    float minimumRadius)
  {
  ...
  private void UpdateNavigationBounds()
  {
    ...
    for (int noteIndex = 1; noteIndex < _noteCount; noteIndex++)
    {
      Vector3 position = _nodes[noteIndex].Position;
      min = Vector3.Min(min, position);
      max = Vector3.Max(max, position);
    }

    _layoutCenter = (min + max) * 0.5f;

    float maxDistance = 0f;
    for (int nodeIndex = 0; nodeIndex < _nodes.Count; nodeIndex++)
      maxDistance = Mathf.Max(maxDistance, (_nodes[nodeIndex].Position - _layoutCenter).magnitude);
