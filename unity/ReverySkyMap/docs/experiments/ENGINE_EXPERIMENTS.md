...
The current engine experiment compares several layout families for note graphs:
...
- Space preservation: `docs/experiments/SPACE_PRESERVATION_EXPERIMENTS.md`
- Material LOD: `docs/experiments/MATERIAL_LOD_EXPERIMENTS.md`
...
Keep durable experiment state outside engine source files: what each candidate
is trying to prove, what has already been observed, which trade-offs matter, and
...
- In the current dirty workspace, `Assets/Scenes/StarScapeScene.unity` wires the
  `StaticLinks` slot to a RecursiveHubs candidate; the latest and most developed
  variant in the eval family is `Engine_RecursiveHubs_v6`.
...
- `CartographerForcesEngine`
  - Family: live force-directed.
  - Runtime mode: `Forces`, ticks every frame.
  - Current read: small-graph baseline. Uses note-tag and note-note springs with
    runtime links. Not the main large-graph candidate.
- `Cartographer25DEngine`
  - Family: date-based chain/depth map.
  - Runtime mode: `Static25D`, no tick.
  - Current read: date-axis engine with known TODOs for labels, radial movement,
    LOD, and preferred camera start. Separate from static-link comparison.
- `Engine_EmptySpheres`
  - Family: static link placement.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: excellent performance and useful fallback. Structural value is
    low, especially for tagless and larger graphs.
- `Engine_FullSimpleStatic_v1`
  - Family: static link placement.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: more compact and slightly more structural than
    `Engine_EmptySpheres`, but mostly superseded by it.
- `Engine_Barnes_v1`
  - Family: Barnes-Hut settled force.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: strong medium-graph shape and cluster separation. Too slow and
    had center-offset flattening. Promising but not viable without fixes.
- `Engine_Barnes_v2_FixedCenterIntoPlate`
  - Family: Barnes-Hut settled force.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: faster and tolerable at small scales, but developed a
    plate-shape/camera-orientation defect and lost some v1 visual strength.
- `Engine_Barnes_v3_Recentered`
  - Family: Barnes v1 derivative.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: targets v1 center drift while preserving v1 visual character.
    Current source note says it may load slightly slower.
- `Engine_Barnes_v4_TaglessIsotropicSeed`
  - Family: Barnes v3 derivative.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: targets the tagless vertical spindle by reseeding tagless
    components isotropically.
- `Engine_Barnes_v5_FasterMidSize`
  - Family: standalone Barnes variant.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: targets 501-2K physical nodes with fewer/rougher settle passes
    while preserving the airy Barnes look and tagless isotropy.
- `Engine_Barnes_v6_ShapeBalanced`
  - Family: Barnes v5 profile wrapper.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: retunes v5 for shape, direct-link structure, and less plate
    behavior while trying to keep speed. Uses reflective profile writes.
- `Engine_Barnes_v7_VolumeGuard`
  - Family: standalone Barnes variant.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: source comments describe it as an improved v1 direction and the
    best force-style structure/readability candidate up to about 2K, but still
    visually glued at 2K and somewhat plate-shaped on tagged graphs.
- `Engine_Grid_v1_1_Relaxed`
  - Family: grid/spatial-hash settled force.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: rejected for poor visual quality, overcrowding, spherical
    blobs, center alignment defect, and weak performance.
- `Engine_Grid_v2_FixedCenterIntoPlate`
  - Family: grid/spatial-hash settled force.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: faster than v1 but still plate-shaped and congested. Needs
    major spacing/shape improvement before more evaluation.
- `Engine_LandmarkStress_v1`
  - Family: landmark/stress embedding.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: pending manual evaluation. Tests a middle path between simple
    static placement and all-node force simulation.
- `Engine_MacroCosmos10K_v1`
  - Family: macro-cluster atlas.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: pending manual evaluation. Designed for 10K maps where macro
    topics and spatial air matter more than local link readability.
- `Engine_StarfieldConstellations10K_v1`
  - Family: constellation atlas.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: pending manual evaluation against `Engine_MacroCosmos10K_v1`
    and RecursiveHubs. Hides local links and emphasizes macro theme
    constellations.
- `Engine_RecursiveHubs_v1`
  - Family: structural hub placement.
  - Runtime mode: `StaticLinks`, no tick.
  - Current read: superseded by v2. Owner notes describe artificial separation,
    long links, edge flattening, and hub/large-map congestion.
- `Engine_RecursiveHubs_v2`
  - Family: iterative hub-cluster layout.
  - Runtime mode: `StaticLinks`, ticks during refinement.
  - Current read: strong cosmos character and useful local links, but serious FPS
    issues at 5K and 10K. Worth improving as a distinct visual direction.
- `Engine_RecursiveHubs_v3`
  - Family: progressive structural hubs.
  - Runtime mode: `StaticLinks`, ticks during construction/refinement.
  - Current read: earlier RecursiveHubs baseline. Owner notes preferred this
    direction over Barnes up to about 2K, but it is now superseded by v6 as the
    active family baseline.
- `Engine_RecursiveHubs_v4`
  - Family: safety wrapper for v3.
  - Runtime mode: `StaticLinks`, inherits v3.
  - Current read: targets the camera/focus risk from progressive construction.
    Source comments conflict: one note says the bug was not fixed and the variant
    is rejected, while the structured block still says pending validation.
- `Engine_RecursiveHubs_v5`
  - Family: v3 animation-control wrapper.
  - Runtime mode: `StaticLinks`, inherits v3.
  - Current read: pending FPS/feel evaluation. Separates construction timing from
    link refinement timing.
- `Engine_RecursiveHubs_v6`
  - Family: RecursiveHubs final-pass wrapper.
  - Runtime mode: `StaticLinks`, inherits v3.
  - Current read: current active family baseline and latest worked-out variant.
    Adds animation timing, adaptive line budgets, edge pruning, and optional
    large-graph visual throttles.
...
- Keep changing experiment state in this journal, not scattered across new prose
  comments in engine files.
...
- Confirm the eval set and owner scoring vocabulary before the next engine
  iteration.
- Reclassify the non-RecursiveHubs candidates as `reference only`, `fallback`, or
  `parked` where that matches the current owner verdict.
...
- Scene wiring currently depends on experiment classes and dirty workspace state.
  A selected engine must have its script and `.meta` tracked before it is treated
  as accepted.
...
- 2026-06-11: Created the original journal from current source comments and
  scene/docs inspection. No engine code or engine comments were changed.
